...
export function ViewPdf(props) {
  ...
  const [referencedWorks, setReferencedWorks] = useState(top100Films);
  ...
  const handleNavigation = async () => {
    try {
      const response = await fetch("https://project700-backend-neo.herokuapp.com/roadmap/papers" + str, {
        method: "GET",
      });
      const json = await response.json();
      const result = json.referenced_works;
      setReferencedWorks(result);
  ...
  //       setPapersData({ course: param1, email: param2, papers: result, token: token });
  //     } catch (error) {
  //       console.log(error);
  //     }
  //   };

  //   handleNavigation("route", history, token);
  ...
  return (
    ...
    <>
      <Autocomplete
        ...
        sx={{ width: "100%" }}
        renderInput={(params) => <TextField {...params} label="Paper" />}
      />
      <br></br>
      <iframe src={pdf} frameBorder="0" height="700px" width="100%"></iframe>
      {/* <PDFViewer
        document={{
          url: pdf,
        }}
